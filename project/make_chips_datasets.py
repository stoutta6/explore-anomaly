...
# training will consist of non-detections (80% split)
# testing will consist of all detections and a subset of non-detections (20% split)
...
figure_dir = '/mnt/c/Users/titor/Dropbox/PC/Documents/data/xview3/figures'

chip_size_pix = 224 # pixels along one edge
target_within_dist_pix = 112 # threshold distance from annotation to chip center to say target is within the chip
target_unsure_dist_pix = 212 # threshold distance from annotation to chip center to say target may be within the chip

dB_thresh = -100 # discard any chips that have any pixels with this low dB intensity or lower

bathy_thresh = -20 # keep only chips that have this low bathymetry in m or lower

chip_holdout = True # set to True to make chips for the holdout set

if chip_holdout:
    target_write_dir = os.path.join(data_dir_root,'chip_sets/holdout/target')
    unsure_write_dir = os.path.join(data_dir_root,'chip_sets/holdout/unsure')
    background_write_dir = os.path.join(data_dir_root,'chip_sets/holdout/background')
else:
    target_write_dir = os.path.join(data_dir_root,'chip_sets/validation/target')
    unsure_write_dir = os.path.join(data_dir_root,'chip_sets/validation/unsure')
    background_write_dir = os.path.join(data_dir_root,'chip_sets/validation/background')
...
for ii in range(len(vh_glob)):
    ...
    # TODO: make chips that have bathymetry or dB below some threshold, cross-reference the ground truth targets, and place in target or background/all folders
    ...
    for jj in tqdm(range(len(chip_centers_row)), desc='Processing chips'):
        for kk in range(len(chip_centers_col)):
            ...
            # reject if bathy is above the threshold
            if bathy_chip > bathy_thresh:
                N_chips_rejected_bathy += 1
                continue
            
            # construct chip as average of vh, vv channels
            chip = np.zeros((chip_size_pix, chip_size_pix, 2),dtype=np.float16)
            chip[:,:,0] = vh_img[chip_edges_row[jj]:chip_edges_row[jj+1], chip_edges_col[kk]:chip_edges_col[kk+1]]
            chip[:,:,1] = vv_img[chip_edges_row[jj]:chip_edges_row[jj+1], chip_edges_col[kk]:chip_edges_col[kk+1]]
            ...
            chip_name = '{}_{}_{}_{}.tif'.format(scene,chip_centers_row[jj],chip_centers_col[kk],label)
            chip_file = os.path.join(chip_write_dir, chip_name)
            io.imsave(chip_file, chip)
